...
pub fn on_unmap_notify(wm: *WM, event: *c.XUnmapEvent) !void {
    ...
    // If the client is still alive rescue it from the frame before
    // on_destroy_notify destroys the frame (which would take the
    // client's subwindow with it).
    if (wm.frames.get(win)) |win_frame| {
        _ = c.XReparentWindow(wm.display, win, wm.root, 0, 0);
        _ = c.XRemoveFromSaveSet(wm.display, win);
        _ = c.XSync(wm.display, 0);
        _ = win_frame;
    }
